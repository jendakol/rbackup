...
|GET* `/download?<metadata>`|int file_version_id|- HTTP 200 with `RBackup-File-Hash` header and file bytes in body<br/>- HTTP 404 if there is no such file available for download|Download file from server, providing it's version id|
|POST* `/upload?<metadata>`|Query: - string file_name<br/>Body: see [file upload](#file-upload) section|- HTTP 200 with [file](#file) in body<br/>- HTTP 400 if calculated hash of received data does not match the provided one<br/>- HTTP 400 if the request is invalid|Upload the file|
